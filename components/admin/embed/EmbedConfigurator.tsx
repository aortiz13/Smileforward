...
function EmbedSettings({ type, onBack }: { type: EmbedType, onBack: () => void }) {
    ...
    const generateCode = () => {
        ...
        if (type === 'inline') {
            // Modificado para mantener proporciones horizontales (16:9) y diseño del frontend
            return `<div style="width:100%; max-width:1200px; margin: 0 auto; aspect-ratio: 16/9; min-height: 500px; overflow:hidden; border-radius: 2rem; border: 1px solid #e5e7eb; background: #ffffff;">
  <iframe 
...
function VisualPreview({ type, brandColor, theme }: { type: EmbedType, brandColor: string, theme: string }) {
    ...
    const MockWidget = ({ className }: { className?: string }) => (
        <div className={`w-full max-w-[800px] aspect-video bg-[${bgColor}] rounded-[2rem] overflow-hidden shadow-2xl border border-[${borderColor}] flex flex-col font-sans relative ${className} transition-all duration-300`} style={{ borderColor }}>
            ...
            <div className="p-8 flex flex-col items-center flex-1 space-y-6 bg-background" style={{ backgroundColor: bgColor }}>
                <div className="w-full h-full border-2 border-dashed rounded-[2rem] flex items-center justify-center bg-muted/10 relative group cursor-default" style={{ borderColor: brandColor }}>
    ...
    if (type === 'inline') {
        return (
            <div className={`w-full max-w-4xl bg-white shadow-sm border p-8 rounded-[2rem] flex flex-col gap-6 ${theme === 'dark' ? 'dark' : ''}`}>
                <div className="space-y-2">
                    <div className="h-8 w-1/3 bg-slate-100 rounded"></div>
                    <div className="h-4 w-2/3 bg-slate-50 rounded"></div>
                </div>
                <div className="border border-dashed border-slate-300 rounded-[2rem] p-4 bg-slate-50/50 flex justify-center">
                    <Card className="relative w-full border border-zinc-100 dark:border-zinc-800 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] overflow-hidden bg-white dark:bg-zinc-900 rounded-[2rem]">
                        <div className="p-0">
                            <WidgetContainer />
                        </div>
                    </Card>
        ...
        return (
            <div className="w-full h-full border border-dashed border-slate-300 rounded-xl relative bg-slate-50 overflow-hidden flex flex-col">
                ...
                <div className="absolute bottom-8 right-8">
                    <div className="h-14 px-6 rounded-full shadow-xl flex items-center gap-3 cursor-default hover:scale-105 transition-transform" style={{ backgroundColor: brandColor }}>
    ...
    if (type === 'click') {
        return (
            <div className="w-full h-full flex flex-col items-center justify-center space-y-8">
                ...
                <button className="px-8 py-4 rounded-lg font-bold text-white shadow-lg transform hover:scale-105 transition-all text-lg" style={{ backgroundColor: brandColor }}>
                    Analizar mi Sonrisa
                </button>
            </div>
        )
